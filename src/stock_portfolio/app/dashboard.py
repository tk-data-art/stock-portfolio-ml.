...
forecast_days = st.sidebar.slider("Forecast Horizon (Days)", min_value=30, max_value=180, value=90, step=30)
rf_rate_input = st.sidebar.number_input("Annual Risk-Free Rate (%)", min_value=0.0, max_value=10.0, value=4.5, step=0.25)
...
if run_button:
    ...
    with st.spinner("Retrieving historical price series..."):
        price_df = collector.fetch_historical_prices(selected_tickers, start_date=start_date, end_date=end_date)
    ...
    for idx, ticker in enumerate(selected_tickers):
        with st.spinner(f"Fitting Prophet model for {ticker}..."):
            ...
            annualized_return = ((target_price - current_price) / current_price) * (365.0 / forecast_days)
    ...
    tab_forecasts, tab_portfolio = st.tabs(["Stock Price Forecasts", "Portfolio Optimization"])
    ...
    with tab_portfolio:
        ...
        fig_ef.add_trace(go.Scatter(
            x=asset_vols.values * 100.0,
            y=mu_series.values * 100.0,
            mode="markers+text",
        ...
        cal_y = [risk_free_rate * 100.0, risk_free_rate * 100.0 + result.sharpe_ratio * (result.volatility * 150.0)]
        fig_ef.add_trace(go.Scatter(
            x=cal_x,
            y=cal_y,
            mode="lines",
        ...
        with col_donut:
            fig_donut = go.Figure(go.Pie(
                ...
                marker=dict(colors=["#2b5c8f", "#4682b4", "#5c9ebb", "#82b7d4", "#a8cfe8"])
            ))
        ...
        with col_table:
            weight_df = pd.DataFrame({
                ...
                "Target Weight": [f"{w * 100.0:.2f}%" for w in result.weights.values()],
                "Annual Volatility": [f"{asset_vols[t] * 100.0:.2f}%" for t in result.weights.keys()],
                "Expected Return": [f"{mu_series[t] * 100.0:.2f}%" for t in result.weights.keys()]
            })
